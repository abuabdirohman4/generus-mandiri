...
export async function fetchTeacherClassMasters(supabase: SupabaseClient, teacherId: string) {
  return await supabase
    ...
    .select('id, class_master_id, custom_class_names, class_masters:class_master_id(id, name, sort_order)')
...
export async function insertTeacherClassMasterAssignments(
  ...
  mappings: Array<{ teacher_id: string; class_master_id: string; custom_class_names?: string[] | null }>
) {
  return await supabase
    .from('teacher_class_masters')
    .insert(mappings)
